...
# -*- coding: utf-8 -*-
"""
Script para realizar el test de normalidad de Shapiro-Wilk.

El test de Shapiro-Wilk es una prueba estadística de normalidad que evalúa
si una muestra de datos proviene de una distribución normal. Es uno de los
tests de normalidad más potentes, especialmente para muestras pequeñas y medianas.

CARACTERÍSTICAS DEL TEST DE SHAPIRO-WILK:
- Es muy potente para muestras pequeñas (3 ≤ n ≤ 50)
- Funciona bien con muestras medianas (hasta n = 5000)
- Es más sensible que otros tests para detectar desviaciones de la normalidad
- Requiere que los datos sean independientes e idénticamente distribuidos
"""
...
def perform_shapiro_wilk_test(data: np.ndarray, significance_level: float = 0.05):
    """
    Realiza el test de normalidad de Shapiro-Wilk.
    
    EL TEST DE SHAPIRO-WILK:
    ------------------------
    El test de Shapiro-Wilk es una prueba estadística de normalidad que evalúa
    si una muestra de datos proviene de una distribución normal. Es uno de los
    tests más potentes para muestras pequeñas y medianas.
    ...
    - a_i son coeficientes calculados a partir de la distribución normal
    ...
    - W cercano a 1 indica mayor evidencia de normalidad
    ...
    # Shapiro-Wilk requiere al menos 3 observaciones
    if n < 3:
        return {
            'statistic': np.nan,
            'p_value': np.nan,
    ...
    # Shapiro-Wilk funciona mejor con muestras hasta 5000
    if n > 5000:
        return {
            'statistic': np.nan,
            'p_value': np.nan,
            ...
            'std': np.std(data, ddof=1),
            'n': n,
            'note': f'Tamaño de muestra muy grande (n={n} > 5000). Shapiro-Wilk es más adecuado para n ≤ 5000.'
        }
    ...
    # scipy.stats.shapiro realiza el test de normalidad de Shapiro-Wilk
    try:
...
def print_test_results(test_result: dict, column_name: str):
    """
    Imprime los resultados del test de normalidad de Shapiro-Wilk.
    """
    print("\n" + "="*90)
    ...
    if 'note' in test_result:
        ...
        print(f"{'Estadístico W':<20} {'p-valor':<20} {'Normal (α=' + str(test_result.get('significance_level', 0.05)) + ')':<20} {'n':<10}")
        print("-" * 90)
...
def process_all_files(base_dir: str, filename: str, column: str = None, 
    ...
    for i, csv_file in enumerate(files, 1):
        ...
        try:
            ...
            # Realizar test
            test_result = perform_shapiro_wilk_test(data, significance_level)
    ...
    for result in all_results:
        if 'error' in result:
            ...
            row = {
                ...
                'Estadistico_W': result.get('statistic', np.nan if NUMPY_AVAILABLE else ''),
...
def print_batch_summary(all_results: list, significance_level: float = 0.05):
    ...
    print("\n" + "="*90)
    print("RESUMEN DE RESULTADOS - TEST DE SHAPIRO-WILK")
    print("="*90)
    ...
    if successful:
        print(f"\n{'─'*90}")
        print(f"{'Carpeta':<25} {'N':<6} {'Estadístico W':<18} {'p-valor':<15} {'Normal':<10}")
        print("-" * 90)
...
def save_results_to_csv(test_result: dict, column_name: str, output_file: str):
    """
    Guarda los resultados del test de Shapiro-Wilk en un archivo CSV.
    """
    output_path = Path(output_file)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    row = {
        ...
        'Estadistico_W': test_result.get('statistic', np.nan if NUMPY_AVAILABLE else ''),
...
def print_explanation():
    """
    Imprime una explicación detallada del test de Shapiro-Wilk.
    """
    print("\n" + "="*90)
    print("EXPLICACIÓN DEL TEST DE SHAPIRO-WILK")
    print("="*90)
    print("""
...
ESTADÍSTICO W:
--------------
El estadístico de Shapiro-Wilk se calcula como:

    W = (Σ a_i · x_(i))² / (Σ (x_i - x̄)²)

donde:
• x_(i) = valores ordenados de la muestra (de menor a mayor)
• a_i = coeficientes calculados a partir de la distribución normal
• x̄ = media de la muestra
• El estadístico W está entre 0 y 1
...
• W cercano a 1 indica mayor evidencia de normalidad
• W cercano a 0 indica menor evidencia de normalidad

VENTAJAS:
---------
✓ Muy potente para muestras pequeñas (3 ≤ n ≤ 50)
✓ Funciona bien con muestras medianas (hasta n = 5000)
✓ Es más sensible que otros tests para detectar desviaciones
✓ Ampliamente utilizado y reconocido en la literatura

LIMITACIONES:
-------------
✗ Requiere al menos 3 observaciones
✗ Funciona mejor con muestras hasta 5000 observaciones
✗ Los datos deben ser independientes e idénticamente distribuidos
✗ Puede ser demasiado estricto para muestras muy grandes

CUÁNDO USARLO:
-------------
• Para validar supuestos de normalidad antes de tests paramétricos
• Cuando tienes muestras pequeñas o medianas (3 ≤ n ≤ 5000)
• Cuando necesitas un test muy potente para detectar desviaciones
• Para validar supuestos en análisis estadísticos (ANOVA, regresión, etc.)

COMPARACIÓN CON OTROS TESTS:
----------------------------
• vs. Kolmogorov-Smirnov: SW es más potente para muestras pequeñas
• vs. Anderson-Darling: SW es más adecuado para muestras pequeñas
• vs. D'Agostino-Pearson: SW compara toda la distribución, no solo momentos
...
def main():
    """
    Función principal que ejecuta el test de normalidad de Shapiro-Wilk.
    """
    parser = argparse.ArgumentParser(
        description='Realiza el test de normalidad de Shapiro-Wilk en un archivo CSV o en todos los archivos de un directorio',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Ejemplos de uso:
  # Comportamiento por defecto (procesa todos los archivos en output/):
  python shapiro_wilk_test.py
  
  # Analizar un archivo específico:
  python shapiro_wilk_test.py output/promedio_06_1_50/promedio_2024_hypervolumes_normality_test.csv
  
  # Analizar todos los archivos en un directorio específico:
  python shapiro_wilk_test.py --batch output/ --filename promedio_2024_hypervolumes_normality_test.csv
  
  # Con opciones adicionales:
  python shapiro_wilk_test.py --batch output/ --alpha 0.01 --output resultados_sw.csv
        """
    )
    parser.add_argument(
        'csv_file',
    ...
    parser.add_argument(
        '--explain', '-e',
        action='store_true',
        help='Muestra una explicación detallada del test de Shapiro-Wilk'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
    ...
    if args.batch:
        ...
        if args.output:
            ...
            # Guardar por defecto
            default_output = Path(args.batch) / "shapiro_wilk_results.csv"
        ...
        # Realizar test
        print(f"\n🔬 Realizando test de normalidad de Shapiro-Wilk (α={args.alpha})...")
        test_result = perform_shapiro_wilk_test(data, args.alpha)
